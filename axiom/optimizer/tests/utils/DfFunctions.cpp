...
namespace facebook::velox::optimizer::test {
  ...
  namespace {
  std::unordered_map<std::string, logical_plan::PlanBuilder::FunctionRewriteHook> functionHooks;
  }
  
  bool featureFuncHook(const std::string& name, const std::vector<ExprPtr> args, logicalPlan::PlanBuilder::FunctionRewrite& rewrite) {
  ...
  void registerFeatureFuncHook(const std::string& name, logical_plan::PlanBuilder::FunctionRewriteHook) {
    featureFuncHooks[name] = hook;
  }


  bool makeRowFromMapHook(const std::string& name, const std::vector<ExprPtr>& args, lp::logical_plan::FunctionRewrite& rewrite) {
