...
class DistributedPlanBuilder : public velox::exec::test::PlanBuilder {
 public:
  ...
  velox::core::PlanNodePtr shufflePartitionedResult(
      const std::vector<std::string>& keys,
      int numPartitions,
      bool replicateNullsAndAny,
      const std::vector<std::string>& outputLayout = {}) override;

  velox::core::PlanNodePtr shuffleBroadcastResult() override;
