...
statement
    ...
    | ALTER TABLE (IF EXISTS)? tableName=qualifiedName
        ALTER (COLUMN)? column=identifier SET NOT NULL_LITERAL                 #alterColumnSetNotNull
    | ALTER TABLE (IF EXISTS)? tableName=qualifiedName
        ALTER (COLUMN)? column=identifier DROP NOT NULL_LITERAL                #alterColumnDropNotNull
    | ALTER TABLE (IF EXISTS)? tableName=qualifiedName
...
columnDefinition
    : identifier type (NOT NULL_LITERAL)? (COMMENT string)? (WITH properties)?
    ;
...
queryTerm
    ...
    | left=queryTerm op=INTERSECT setQuantifier? right=queryTerm         #setOperation
    | left=queryTerm op=(UNION | EXCEPT) setQuantifier? right=queryTerm  #setOperation
    ;
...
predicate[antlr4::ParserRuleContext* value]
    ...
    | IS NOT? NULL_LITERAL                                                        #nullPredicate
